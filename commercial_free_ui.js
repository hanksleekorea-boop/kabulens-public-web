(function(){
  ...
  var storageReadOnly=false,content,advancedContent,currentPack,state=load();
  function el(id){return document.getElementById(id);}
  function esc(value){return String(value==null?'':value).replace(/[&<>"']/g,function(c){return{'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c];});}
  function load(){try{var raw=localStorage.getItem(KEY),value=JSON.parse(raw||'{}');if(window.StockScannerReliability){var entries={};entries[KEY]=raw;window.StockScannerReliability.validateEntries(entries);}storageReadOnly=false;return Commercial.state(value);}catch(error){storageReadOnly=true;return Commercial.state();}}
  function persist(){try{if(storageReadOnly)throw new Error('저장된 원본을 읽지 못했습니다. 전체 자료 백업으로 원본을 보관한 뒤 복원하세요.');localStorage.setItem(KEY,JSON.stringify(state));}catch(error){state=load();throw error;}}
  ...
  function makePack(){
    var selected=selection(),ctx=window.StockScannerReportContext;
    if(ctx&&(selected.securityIds.indexOf(ctx.securityId)<0||selected.horizon!==ctx.horizon||selected.asOf!==ctx.asOf))throw new Error('현재 보고서와 연구 조건이 다릅니다. 고급 연구에서 종목·기간·기준일을 먼저 맞추세요.');
  ...
  function wire(){
    ...
    el('commercialJournalForm').addEventListener('submit',function(event){event.preventDefault();try{state=Commercial.addJournal(state,{hypothesis:el('commercialHypothesis').value,disconfirm:el('commercialDisconfirm').value,nextCheck:el('commercialNextCheck').value,reportRef:window.StockScannerReportContext||null});persist();this.reset();el('commercialJournalStatus').textContent='판단과 반증 조건을 이 기기에 저장했습니다.';renderJournal();renderChecklist();}catch(error){el('commercialJournalStatus').textContent='저장하지 못했습니다: '+error.message;}});
    ...
    window.addEventListener('stockscanner-delete-all',function(){state=Commercial.state();currentPack=null;el('commercialBoundaryConfirm').checked=false;render();});
    window.addEventListener('stockscanner-data-restored',function(){state=load();currentPack=null;el('commercialBoundaryConfirm').checked=state.boundaryConfirmed;render();});
  }
