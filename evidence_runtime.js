(function(root,factory){
  ...
  var DATA_STATES=['REAL_ALLOWED','SYNTHETIC','LINK_ONLY','BLOCKED','STALE','MISSING'];
  var DATA_STATE_COPY={
    REAL_ALLOWED:'허가된 실제 자료',
    SYNTHETIC:'합성 시험자료',
    LINK_ONLY:'공식 링크만 제공',
    BLOCKED:'사용권 또는 검토 차단',
    STALE:'확인일 경과',
    MISSING:'자료 상태 미확인'
  };
  ...
  function normalizeDataState(value){return DATA_STATES.indexOf(text(value))>=0?text(value):'MISSING';}
  function normalize(input,now){
    var card=Object.assign({status:HOLD,confidence:'미확인',holdReason:'근거 검토 전에는 판정을 공개하지 않습니다.',dataState:'MISSING',licenseState:'UNKNOWN'},input||{});
    card.dataState=normalizeDataState(card.dataState);
    ...
    if(card.dataState==='SYNTHETIC'){state='SYNTHETIC';reason='합성 시험자료는 실제 공시 판정으로 공개하지 않습니다.';}
    else if(card.dataState==='LINK_ONLY'){state='LINK_ONLY';reason='원문은 공식 사이트 링크에서만 확인할 수 있습니다.';}
    else if(card.dataState==='BLOCKED'){state='LICENSE_BLOCKED';reason='자료 사용권 또는 독립 검토가 끝나지 않아 판정을 공개하지 않습니다.';}
    else if(card.dataState==='STALE'){state='STALE';reason='자료 상태가 오래되어 판정을 공개하지 않습니다.';}
    else if(card.dataState==='MISSING'){state='MISSING_DATA_STATE';reason='자료 상태가 확인되기 전에는 판정을 공개하지 않습니다.';}
    else if(card.licenseState!=='ALLOWED'){state='LICENSE_BLOCKED';reason='허가 상태가 확인되지 않아 판정을 공개하지 않습니다.';}
    else if(!complete){state='MISSING_EVIDENCE';reason='필수 근거가 부족합니다.';}
  ...
  function render(input,now){
    ...
    var sourceLink=(result.state==='READY'||result.state==='LINK_ONLY')&&/^https:\/\//.test(text(card.sourceUrl))?'<p><a href="'+escapeHtml(card.sourceUrl)+'" target="_blank" rel="noopener noreferrer">공식 원문 위치 열기</a></p>':'';
    var recovery=result.state==='SOURCE_UNAVAILABLE'?'<p class="evidence hold">원문 연결을 다시 확인하거나 공식 사이트에서 문서명을 검색하세요.</p>':'';
    return '<article class="evidence-card" tabindex="0" data-evidence-status="'+escapeHtml(card.status)+'" data-evidence-state="'+escapeHtml(result.state)+'" data-data-state="'+escapeHtml(card.dataState)+'"><span class="tag">근거 검토 · '+escapeHtml(card.verdictId||'E03')+'</span><h3>'+escapeHtml(card.title||'약속 이행 근거 카드')+'</h3><p class="evidence data-state">자료 상태: '+escapeHtml(DATA_STATE_COPY[card.dataState])+'</p><p class="score '+(card.status===HOLD?'warn':'up')+'">'+escapeHtml(card.status)+'</p>'+detail+sourceLink+recovery+'<p class="evidence">'+escapeHtml(card.holdReason||'')+'</p></article>';
  }
  ...
  function attach(doc){
    ...
    node.innerHTML=LAUNCH_ENGINES.map(function(item){return render({verdictId:item[0],title:item[1]+' 판정',status:HOLD,dataState:'BLOCKED',licenseState:'UNKNOWN',confidence:'미확인',holdReason:'실제 공시·사용권·독립 검토가 끝나기 전에는 결과를 공개하지 않습니다.'});}).join('');
  ...
  ready();return {REQUIRED:REQUIRED,HOLD:HOLD,MAX_AGE_DAYS:MAX_AGE_DAYS,STATES:STATES,DATA_STATES:DATA_STATES,DATA_STATE_COPY:DATA_STATE_COPY,LAUNCH_ENGINES:LAUNCH_ENGINES,normalizeDataState:normalizeDataState,normalize:normalize,render:render,renderUiState:renderUiState,attach:attach};
});
