...
test('Stock Scanner public release is responsive functional and accessible',async({page,request,baseURL},testInfo)=>{
  ...
  await expect(page.locator('#mobileReportBody')).toContainText('예시 계산의 성격');
  await expect(page.locator('#mobileReportBody')).toContainText('이 방법, 쉽게 이해하기');
  await expect(page.locator('#mobileReportBody')).toContainText('30초 설명');
  ...
  await expect(page.locator('#mobileReportBody')).toContainText('선택 방법 비교·민감도');
  await expect(page.locator('#mobileReportBody')).toContainText('실제 과거 성적은 제공하지 않습니다.');
  await expect(page.locator('#mobileReportBody')).not.toContainText('NOT_RUN');
  await expect(page.locator('#mobilePrintReport')).toBeVisible();
